...
class TopNodeFixer(AIndependentFixer):
    def fix_tree(self) -> tuple[MappedDecisionTree, list[int]]:
        ...
        """
        top_faulty_node_index = min(self.faulty_nodes, key=lambda node_index: self.mapped_tree.get_node(node_index).depth)
        data_reached_top_faulty_node = self._filter_data_reached_fault(top_faulty_node_index)
        self.fix_faulty_node(top_faulty_node_index, data_reached_top_faulty_node)
